...
export function AuthGate({ children }: { children: ReactNode }) {
  ...
  if (!user) return <main className="auth-page"><div className="auth-shape auth-shape-one"/><div className="auth-shape auth-shape-two"/><section className="auth-card"><div className="auth-brand">{instituteMark}<img className="auth-wordmark" src={`${import.meta.env.BASE_URL}school-of-music-wordmark.png`} alt="PGVIM School of Music"/></div><p className="eyebrow">MY PGVIM · BM 2567</p><h1>ตัวช่วยวางแผนการเรียนของคุณ</h1><p>ตรวจสอบหน่วยกิต วิชาที่ยังขาด และวางแผนการเรียนด้วยข้อมูลของคุณเอง</p><button className="google-button" onClick={()=>void signIn()}><span>G</span>เข้าสู่ระบบด้วย Google ของสถาบัน</button><small>ใช้ได้เฉพาะบัญชีที่ลงท้ายด้วย <b>@{INSTITUTION_DOMAIN}</b></small>{error&&<div className="auth-error">{error}</div>}</section></main>;
  return children;
}
